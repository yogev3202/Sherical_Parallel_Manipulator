function [output, mem] = mpc_nmpcsolver(input, settings, mem, opt)
    ...
    while(mem.sqp_it < mem.sqp_maxit  &&  StopCrit > mem.kkt_lim && mem.alpha>1E-3 ) % RTI or multiple call
        if (strcmp(model_type,'Dynamic_Model_QLPV_V2') || ...
            strcmp(model_type,'Dynamic_Model_QLPV'))
            input.od(1:6,:) = [X0,input.x(1:6,2:end)];
        end
        ...
        if opt.nonuniform_grid
            ...
            if strcmp(opt.qpsolver, 'qpoases_mb')
                ...
                % temp = input.od(1:6,:) + input.x;
                % input.x = temp;
                % input.x0 = input.x0 + input.para0(1:6);
...
%                 qp_generation_tac(input, settings, mem);
        ...
        switch opt.qpsolver
            case 'qpoases_sequence'              
                [tQP,mem] = mpc_qp_solve_qpoases_sequence(settings,mem);
            case 'qpoases'    
