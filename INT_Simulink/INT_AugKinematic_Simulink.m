...
CTR_PATH = 'Kinematic_Simulation/Aug_Kinematic_Model/CTR_PARAMETERS';
...
Qz = CTR_MASK.Parameters(1, 7).Value;
Rv = CTR_MASK.Parameters(1, 8).Value;
SPM_Mdl = CTR_MASK.Parameters(1, 9).Value;
GenMex = CTR_MASK.Parameters(1, 10).Value;
St0 = CTR_MASK.Parameters(1, 11).Value;
EAr_0 = CTR_MASK.Parameters(1, 12).Value;
Kp = CTR_MASK.Parameters(1, 13).Value;
Kv = CTR_MASK.Parameters(1, 14).Value;
Ki = CTR_MASK.Parameters(1, 15).Value;

Nx = str2num(Nx); Nu = str2num(Nu); Ny = str2num(Ny);
dt = str2num(dt); N_mpc = str2num(N_mpc); Qv = str2num(Qv);
Rv = str2num(Rv); St0 = str2num(St0); EAr_0 = str2num(EAr_0);
Qz = str2num(Qz); Kp = str2num(Kp); Ki = str2num(Ki); Kv = str2num(Kv);
...
if (strcmp(GenMex,'on'))
    ...
    switch SPM_Mdl
        ...
        case 'Kinematic Model'
            settings.model='Kinematic_Model'; 
            
        case 'Augmented Kinematic Model'
            settings.model='Aug_Kinematic_Model';
    end
...
x0 = [St0;EAr_0-St0]*(pi/180);     %[0;0;0] * (pi/180);    
u0 = zeros(nu,1); 
z0 = zeros(nz,1);
