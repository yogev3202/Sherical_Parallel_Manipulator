...
CTR_PATH = 'DynamicValidation/Dynamic_Model_PID/CTR_PARAMETERS';
...
Q = CTR_MASK.Parameters(1, 6).Value;
R = CTR_MASK.Parameters(1, 7).Value;
GenMex = CTR_MASK.Parameters(1, 8).Value;

x0_SPM = CTR_MASK.Parameters(1, 9).Value;

Nx = str2num(Nx); Nu = str2num(Nu); Ny = str2num(Ny);
dt = str2num(dt); N_mpc = str2num(N_mpc); Q = str2num(Q);
R = str2num(R);x0_SPM = str2num(x0_SPM);



if (strcmp(GenMex,'on'))

    settings.model='Err_Dynamic_Model'; 
...
%% options
opt.hessian='Generalized_Gauss_Newton';  % 'Gauss_Newton', 'Generalized_Gauss_Newton'
...
lb_x = -[1000;40;40]*(pi/180);
ub_x =  [1000;40;40]*(pi/180);

% upper and lower bounds for controls (=nbu)           
lb_u = -500 * ones(3,1) *(pi/180);
ub_u =  500 * ones(3,1) *(pi/180);
                       
% upper and lower bounds for general constraints (=nc)
lb_g = [ones(3,1)*0.1;0.2];
ub_g = [ones(3,1)*2;1000];           
lb_gN = [ones(3,1)*0.1;0.2];
ub_gN = [ones(3,1)*2;1000]; 
