...
CTR_PATH = 'SIMSCAPE_SIMULATION/Dynamic_Model/CTR_PARAMETERS';
...
x0 = CTR_MASK.Parameters(1, 10).Value;
ref0 = CTR_MASK.Parameters(1, 11).Value;


Nx = str2num(Nx); Nu = str2num(Nu); Ny = str2num(Ny);
dt = str2num(dt); N_mpc = str2num(N_mpc); Qv1 = str2num(Qv1);
Qv2 = str2num(Qv2);Rv = str2num(Rv);
x0 = str2num(x0); ref0 = str2num(ref0);
...
THETAv = SPMIK_Fcn(x0(1:3) * (pi/180),'---');
...
Qx2 = Qv2;
Qu = Rv;
W=repmat([Qx1 Qx2 Qu]',1,N);
WN=W(1:nyN,1);

% upper and lower bounds for states (=nbx)
Xlb = -100 * (pi/180) * ones(3,1);
Xub = 100 * (pi/180) * ones(3,1);
lb_x = dr - Xub;
ub_x =  dr-Xlb;
