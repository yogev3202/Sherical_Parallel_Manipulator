...
CTR_PATH = 'SIMSCAPE_SIMULATION/Dynamic_Model_PID/CTR_PARAMETERS';
...
Qz = CTR_MASK.Parameters(1, 8).Value;
Rv = CTR_MASK.Parameters(1, 9).Value;
GenMex = CTR_MASK.Parameters(1, 10).Value;

Kp = CTR_MASK.Parameters(1, 11).Value;
Kv = CTR_MASK.Parameters(1, 12).Value;
Ki = CTR_MASK.Parameters(1, 13).Value;

x0_SPM = CTR_MASK.Parameters(1, 14).Value;
ref0 = CTR_MASK.Parameters(1, 15).Value;


Nx = str2num(Nx); Nu = str2num(Nu); Ny = str2num(Ny);
dt = str2num(dt); N_mpc = str2num(N_mpc); Qv1 = str2num(Qv1);
Qv2 = str2num(Qv2); Qz = str2num(Qz);Rv = str2num(Rv);
x0_SPM = str2num(x0_SPM); ref0 = str2num(ref0);
