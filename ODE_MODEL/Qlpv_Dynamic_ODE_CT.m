function states_dot = Qlpv_Dynamic_ODE_CT(states,controls,params)
...
%% STATES AND INPUTS
    ...
    psi   = p1;teta  = p2;  phi   = p3;
    dpsi  = p4;dteta = p5;  dphi  = p6;
...
%% Calculation of CORIOLIS MATRIX (C)
    ...
    %% Leg 1
    W11 = Jw11*[dpsi;dteta;dphi]; % Angular Velocity Proximal
    W21 = Jw21*[dpsi;dteta;dphi]; % Angular Velocity Distal    
    ...
    %% Leg 2
    W12 = Jw12*[dpsi;dteta;dphi]; % Angular Velocity Proximal
    W22 = Jw22*[dpsi;dteta;dphi]; % Angular Velocity Distal  
    ...
    %% Leg 3
    W13 = Jw13*[dpsi;dteta;dphi]; % Angular Velocity Proximal
    W23 = Jw23*[dpsi;dteta;dphi]; % Angular Velocity Distal  
...
%% Output ---------------------------    
    ...
    % G_1 = Gv(1)*(1/(psi+epsilon));
    % G_2 = Gv(2)*(1/(teta+epsilon));
    % G_3 = Gv(3)*(1/(phi+epsilon));   
    ...
    G_1 = Gv(1)*(SC(p1,Eps)/sin(p1+Eps));
    G_2 = Gv(2)*(SC(p2,Eps)/sin(p2+Eps));
    G_3 = Gv(3)*(SC(p3,Eps)/sin(p3+Eps));
    ...
    % states_dot = Ac*[X1+Eps*ones(3,1);X2] + Bc*u;
    states_dot = Ac*[X1;X2] + Bc*u;
