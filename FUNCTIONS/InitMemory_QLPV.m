function [mem] = InitMemory_QLPV(settings, opt, input)
    ...
    switch opt.qpsolver
        case 'qpoases'   
            ...
            % mem.qpoases_opt.enableRamping = 1;
            % mem.qpoases_opt.enableFlippingBounds = 1;
            mem.qpoases_opt.terminationTolerance = 1e-3;
...
%             mem.qpoases_opt = qpOASES_options('default');
        case 'quadprog_dense'
            ...
            mem.quadprog_opt.Algorithm = 'active-set';
            mem.quadprog_opt.Display = 'off';
            mem.quadprog_opt.OptimalityTolerance = 1e-4;
            mem.quadprog_opt.ConstraintTolerance = 1e-4;
            mem.quadprog_opt.StepTolerance = 1e-4;
            mem.quadprog.X0 = zeros(N*nu,1);
        case 'hpipm_sparse'
            mem.mu0=1e4;
            mem.max_qp_it = 30;
            mem.pred_corr = 1;
        ...
        case 'ipopt_dense'
            ...
            %             'never-monotone-mode','accept_every_trial_step','yes','acceptable_iter',10,...
            %             'bound_relax_factor',1e-6);

        ipopt_opts=ipoptset('constr_viol_tol',1e-2,'acceptable_tol',1e-2,'hessian_constant','yes',...
      'acceptable_iter',1,'jac_d_constant','yes','accept_every_trial_step','yes',...
      'max_soc',2,'watchdog_trial_iter_max',1, ...
       'jac_c_constant','yes','bound_relax_factor',1e-2);
        ...
        case 'ipopt_sparse'
         ...
         ipopt_opts=ipoptset('constr_viol_tol',1e-3,'acceptable_tol',1e-3,'acceptable_iter',10);          
...
%             mem.B_tab=[(16-sqrt(6))/36;(16+sqrt(6))/36;1/9];
    ...
    if strcmp(opt.RTI,'yes')
        mem.sqp_maxit = 5;         % use RTI
    else       
        mem.sqp_maxit = 15;      % maximum number of iterations for each sampling instant
    end
    mem.kkt_lim = 1e-5;          % tolerance on optimality
    mem.mu_merit=0;              % initialize the parameter
    mem.eta=1e-4;                % merit function parameter
    mem.tau=0.8;                 % step length damping factor
    mem.mu_safty=0.4;              % constraint weight update factor (for merit function)
