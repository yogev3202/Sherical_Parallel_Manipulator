...
%% Continuous-time nonlinear dynamic model of a spm
...
function [Ad,Bd,Wd] = DynMatQlmpcDT(params,Ts)
...
%% Calculation of CORIOLIS MATRIX (C)
    ...
    %% Leg 1
    W11 = Jw11*[p4;p5;p6]; % Angular Velocity Proximal
    W21 = Jw21*[p4;p5;p6]; % Angular Velocity Distal    
    ...
    %% Leg 2
    W12 = Jw12*[p4;p5;p6]; % Angular Velocity Proximal
    W22 = Jw22*[p4;p5;p6]; % Angular Velocity Distal  
    ...
    %% Leg 3
    W13 = Jw13*[p4;p5;p6]; % Angular Velocity Proximal
    W23 = Jw23*[p4;p5;p6]; % Angular Velocity Distal  
...
%% Output ---------------------------    
    ...
    Ac = [zeros(3),eye(3);zeros(3),-Mm^(-1)*Cm];
    Bc = [zeros(3);Mm^(-1)*Ja'];
    Wc = [zeros(3,1);-Mm^(-1)*Gv];


    Ad = 1/24*Ac^4*Ts^4 + 1/6*Ac^3*Ts^3 + 1/2*Ac^2*Ts^2 + Ac*Ts + eye(6);
    Bd = 1/24*Ac^3*Bc*Ts^4 + 1/6*Ac^2*Bc*Ts^3 + 1/2*Ac*Bc*Ts^2 + Bc*Ts;
    Wd = 1/24*Ac^3*Wc*Ts^4 + 1/6*Ac^2*Wc*Ts^3 + 1/2*Ac*Wc*Ts^2 + Wc*Ts;  
