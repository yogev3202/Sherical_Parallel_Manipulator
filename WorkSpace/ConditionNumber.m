function CN = ConditionNumber(X,Case)
%% UNIT CONVERTION
dg2rd = pi/180;
%% DEFINE PARAMETERS
...
%%  Jacobian Singular
Am = [cross(w1v,v1v)';cross(w2v,v2v)';cross(w3v,v3v)'];
B1 = cross(w1v,u1v)'*v1v;
B2 = cross(w2v,u2v)'*v2v;
B3 = cross(w3v,u3v)'*v3v;
Bm = diag([B1,B2,B3]);
...
%         Norm1 = sqrt(trace(JIK'*((1/3)*eye(3))*JIK));
%         Norm2 = sqrt(trace((JIK^(-1))'*((1/3)*eye(3))*(JIK)^(-1)));

        Norm1 = sqrt(trace(J'*((1/3)*eye(3))*J));
        Norm2 = sqrt(trace((J^(-1))'*((1/3)*eye(3))*(J)^(-1)));

        
    case 'FRK'
        Norm1 = sqrt(trace(JDK'*((1/3)*eye(3))*JDK));
        Norm2 = sqrt(trace((JDK')^(-1)*((1/3)*eye(3))*(JDK)^(-1)));
