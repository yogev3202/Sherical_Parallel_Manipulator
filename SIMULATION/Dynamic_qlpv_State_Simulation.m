...
opt.qpsolver        = 'qpoases';    %'qpoases'; qpoases_sequence ipopt_dense
...
settings.model = 'Dynamic_Model_qlpv';
D2R = pi/180;
R2D = 180/pi;
...
X0 = [0;0;0;0;0;0] * D2R;
settings.x0 = X0;
[input, data , Ref] = InitData_QLPV(settings,0);
...
while time(end) < Tf
   ...
   state_k = xk;
   settings.x0 = state_k;
   [input, data , Ref] = InitData_QLPV(settings, Time);
   ...
   input.yN = data.REF(1:nyN)';
    ...
    % CPT = [CPT; Time_CPT, tshooting, tcond, tqp];
