...
opt.qpsolver        = 'qpoases';    %'qpoases';  osqp_sparse; ipopt_partial_sparse qpoases_sequence
opt.hotstart        = 'yes'; %'yes','no' (only for qpoases, use 'no' for nonlinear systems)
...
opt.RTI             = 'yes'; % if use Real-time Iteration 'yes','no
opt.warm_start      = 'no';
%% available qpsolver
...
X0 = [0;0;0;0;0;0] * D2R;
settings.x0 = X0;
[input, data , Ref] = InitData(settings,0);
...
while time(end) < Tf
   ...
   input.yN = data.REF(1:nyN)';
    ...
    % display(xk(4:6)*180/pi);

    Dis = Discriminants(xk);
    CI = ConditionIndex(xk);
    Err = (r - xk(1:3))*180/pi;
    display([xk(1:6)*180/pi;Dis;CI;Err]');  
    % display([xk(1:6)*180/pi;Dis;CI]');  
    % display(Err');  
    % display(uk*1000');  
    % [A,B1,B2,B3] = JACOBIANS(xk(1:3));
    % display([det(A),B1,B2,B3]);

   % if (Dis(1) <= 0.201 || Dis(2) <= 0.201 || Dis(3) <= 0.201)
   % 
   %      if (Flag_Bound == 1)
   %          opt.qpsolver = 'ipopt_dense';
   %          mem_old = mem;
   %          mem = UpdateMemory(settings, opt, input , mem_old);
   %          Flag_Bound = 0;
   %      end
   % 
   %  end
