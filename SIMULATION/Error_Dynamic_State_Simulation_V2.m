...
while time(end) < Tf
   ...
   input.yN = data.REF(1:nyN)';
    ...
    [t1,y1] = ode45(@(t1,y1) z_dot, [0 Ts], Zk);
    xkp1  = y(end,:)';
    Zkp1 = y1(end,:)';
    xk = xkp1;
    Zk = Zkp1;
   ...
   state_k = [Ek;Zk];    
    ...
    % params = [r;dr;ddr];
    % state_dot= Dynamic_ODE_CT_V2(state_k,uk,params);
    % [t,y] = ode45(@(t,y) state_dot, [0 Ts], state_k);
    % state_kp1 = y(end,:)';
    % state_k = state_kp1;
    % xk = state_k(1:6);
    % Zk = state_k(7:9);
    ...
    display([(r - xk(1:3)),xk(4:6)]*180/pi);
